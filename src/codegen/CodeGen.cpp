...
void CodeGen::gen_epilogue() {
    ...
    append_inst(context.func->get_name() + "epilogue", ASMInstruction::Label);
    if (IS_IMM_12(static_cast<int>(context.frame_size))) {
        append_inst("addi.d $sp, $sp, " +
                    std::to_string(static_cast<int>(context.frame_size)));
...
void CodeGen::gen_ret() {
    ...
    auto *type = context.func->get_return_type();
    if(type->is_void_type())
    {
        append_inst(ADDI WORD, 
                    {"$a0, $zero", "0"});
    }
    ...
    append_inst("b " + context.func->get_name() + "epilogue");
    // throw not_implemented_error{__FUNCTION__};
}
...
void CodeGen::gen_br() {
    ...
    if (branchInst->is_cond_br()) {
        ...
        append_inst("beq", 
                    {"$zero", "$zero", label_name(falsebranchbb)});
        // throw not_implemented_error{__FUNCTION__};
    } else {
...
void CodeGen::gen_alloca() {
    ...
    auto *alloca_ptr = static_cast<AllocaInst*>(context.inst);
    int64_t offset = context.offset_map.at(context.inst);
    offset -= alloca_ptr->get_alloca_type()->get_size();
...
void CodeGen::gen_load() {
    ...
    if (type->is_float_type()) {
        ...
        if(type->is_int1_type())
        {
            append_inst(LOAD BYTE,
                        {"$t0", "$t0", "0"});
        }
        else if(type->is_int32_type())
        {
            append_inst(LOAD WORD,
                        {"$t0", "$t0", "0"});
        }
        else {
            append_inst(LOAD DOUBLE, 
                        {"$t0", "$t0", "0"});
        }
        store_from_greg(context.inst, Reg::t(0));
        // throw not_implemented_error{__FUNCTION__};
...
void CodeGen::gen_store() {
    ...
    if (type->is_float_type()) {
        load_to_freg(context.inst->get_operand(0), FReg::ft(0));
        append_inst(FSTORE SINGLE, 
                    {FReg::ft(0).print(), "$t1", "0"});
    } else {
        load_to_greg(context.inst->get_operand(0), Reg::t(0));
        if(type->is_int1_type()) {
            append_inst(STORE BYTE, 
                        {"$t0", "$t1", "0"});
        }
        else if(type->is_int32_type()) {
            append_inst(STORE WORD, 
                        {"$t0", "$t1", "0"});
        } 
        else {
            append_inst(STORE DOUBLE,
                        {"$t0", "$t1", "0"});
        }
    }
    // throw not_implemented_error{__FUNCTION__};
...
void CodeGen::gen_icmp() {
    // TODO: 处理各种整数比较的情况
    std::string true_label = "." + context.func->get_name() + "_" + context.inst->get_name() + "true";
    std::string finish_label = "." + context.func->get_name() + "_" + context.inst->get_name() + "finish";
    load_to_greg(context.inst->get_operand(0), Reg::t(0));
    load_to_greg(context.inst->get_operand(1), Reg::t(1));
    switch (context.inst->get_instr_type()) {
        case Instruction::ge:
    ...
    append_inst("b " + finish_label);

    append_inst(true_label, ASMInstruction::Label);
    append_inst("addi.d $t0, $zero, 1");

    append_inst(finish_label, ASMInstruction::Label);
    store_from_greg(context.inst, Reg::t(0));
    // throw not_implemented_error{__FUNCTION__};
}
...
void CodeGen::gen_fcmp() {
    ...
    switch (context.inst->get_instr_type()) {
        case Instruction::fge:
            append_inst("fcmp.sle.s",
                        {"$fcc0", "$ft1", "$ft0"});
            break;
        case Instruction::fgt:
            append_inst("fcmp.slt.s",
                        {"$fcc0", "$ft1", "$ft0"});
            break;
        ...
        case Instruction::feq:
            append_inst("fcmp.seq.s",
                        {"$fcc0", "$t0", "$t1"});
            break;
        case Instruction::fne:
            append_inst("fcmp.sne",
                        {"$fcc0", "$t0", "$t1"});
        default:
            break;
    }
...
void CodeGen::gen_call() {
    ...
    for(unsigned int i = 1; i < context.inst->get_num_operand(); i++)
    {
        ...
        if(type->is_float_type())
        {
            load_to_freg(context.inst->get_operand(i), FReg::fa(i - 1));
        }   
        else
        {
            load_to_greg(context.inst->get_operand(i), Reg::a(i - 1));
        }
    }
...
void CodeGen::gen_gep() {
    // TODO: 计算内存地址
    load_to_greg(context.inst->get_operand(0), Reg::t(0));
    auto *type = context.inst->get_operand(0)->get_type()->get_pointer_element_type();
    append_inst(std::to_string(context.inst->get_num_operand()), ASMInstruction::Comment);
    if(type->is_array_type()) {
        int size = type->get_array_element_type()->get_size();
        load_large_int64(size, Reg::t(1));
        append_inst("1_" + std::to_string(size), ASMInstruction::Comment);
        load_to_greg(context.inst->get_operand(2), Reg::t(2));
    }
    else {
        int size = type->get_size();
        load_large_int64(size, Reg::t(1));
        append_inst("2_" + std::to_string(size), ASMInstruction::Comment);
        load_to_greg(context.inst->get_operand(1), Reg::t(2));
    }
    append_inst(MUL DOUBLE,
                {"$t1", "$t1", "$t2"});
    append_inst(ADD DOUBLE,
                {"$t0", "$t0", "$t1"});
    store_from_greg(context.inst, Reg::t(0));
    // throw not_implemented_error{__FUNCTION__};
}
...
void CodeGen::gen_sitofp() {
    ...
    LOG(DEBUG) << "si2fp";
    load_to_greg(inst->get_operand(0), Reg::t(0));
    append_inst(GR2FR WORD,
                {"$ft0", "$t0"});
    append_inst("ffint.s.w", 
                {"$ft0", "$ft0"});
    store_from_freg(context.inst, FReg::ft(0));
    // throw not_implemented_error{__FUNCTION__};
}
...
void CodeGen::gen_fptosi() {
    ...
    LOG(DEBUG) << "fp2si";
    load_to_freg(inst->get_operand(0), FReg::ft(0));
    append_inst("ftintrz.w.s",
                {"$ft0", "$ft0"});
    append_inst(FR2GR SINGLE, 
                {"$t0", "$ft0"});
    store_from_greg(context.inst, Reg::t(0));
    // throw not_implemented_error{__FUNCTION__};
}
