...
void translate_main(CodeGen *codegen) {
    ...
    offset_map["array"] = offset_map["%op0"] - 40; // TODO: 请填空
    ...
    // 你需要思考 %op1 的偏移量是多少字节
    offset_map["%op1"] = offset_map["array"] - 8; // TODO: 请填空
    // 获得数组的地址, 将其写入 $t0 中
    ...
    // 你可以手动计算 gep 的值, 或者使用指令计算 gep 的值
    // 你需要思考 %op2 的偏移量是多少字节
    offset_map["%op2"] = offset_map["%op1"] - 8; // TODO: 请填空
    // 获得数组的地址, 将其写入 $t0 中
    codegen->append_inst("ld.d",
                         {"$t0", "$fp", std::to_string(offset_map["%op0"])});
    ...
    offset_map["%op3"] = offset_map["%op2"] - 4; // TODO: 请填空
    ...
    offset_map["%op4"] = offset_map["%op3"] - 4; // TODO: 请填空
    ...
    offset_map["%op5"] = offset_map["%op4"] - 4; // TODO: 请填空
    ...
    codegen->append_inst("ld.w",
                         {"$t0", "$fp", std::to_string(offset_map["%op5"])});
    codegen->append_inst("move $a0, $t0");
